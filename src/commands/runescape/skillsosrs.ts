...
export default new Command({
  ...
  run: async ({ interaction, args }) => {
    ...
    await hiscores
      .getStats(rsn)
      .then(async (res) => {
        const stats = res as Player;
        const embed = await createEmbed(rsn, stats);
...
const createEmbed = async (rsn: string, stats: Player) => {
  ...
  if (!stats.main) {
    embed.setDescription("User not found on hiscores");
    return embed;
  }
