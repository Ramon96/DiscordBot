...
import fetchPokemon from "../../helpers/api/pokemon/fetchPokemon";
// @ts-ignore
import { getTypeWeaknesses } from "poke-types";
import getKeyByValue from "../../helpers/functions/keyByValue";
...
export async function execute(interaction: CommandInteraction) {
  ...
  if (!pokemon) return interaction.reply("Please provide a pokemon.");
  ...
  try {
    ...
    const type = data.types.map((t: any) => t.type.name).join(", ");
    const stats = data.stats
      .map((s: any) => `${s.stat.name}: ${s.base_stat}`)
      .join("\n");
    const evs = data.stats
      .map((s: any) => `${s.stat.name}: ${s.effort}`)
      .join("\n");
    let typeChart =
      data.types.length > 1
        ? getTypeWeaknesses(data.types[0].type.name, data.types[1].type.name)
        : getTypeWeaknesses(data.types[0].type.name);
    const weakness2x =
    ...
    const immunity =
      getKeyByValue(typeChart, 0).length > 0
        ...
        : "No immunity";
    const hiddenAbility = data.abilities.find((a: any) => a.is_hidden)
      ? data.abilities.find((a: any) => a.is_hidden).ability.name
      : "none";
    const abilities = data.abilities
      .filter((a: any) => a.is_hidden == false)
